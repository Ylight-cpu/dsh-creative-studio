...
图片侧抠图 / 加文字 / 贴 logo / **改图（局部重绘去杂物）** / 合成 / 批量规格化 / 质检；视频侧素材通览 / 文案脚本 / 剪辑合成 / 字幕 / 配乐 / 封面 / 平台导出。
...
| **T3（可选）** | + GPU / 代理 | `onnxruntime-gpu` 加速、更快模型下载（`-Gpu` / `-Proxy`） |
...
```
package.json / cordis.patch.yml / lib/index.js   插件外壳（也可作为 DSH profile bundle 安装）
skills/image-studio/SKILL.md                     美工技能说明（工作流/命令表/验收/红线）
skills/video-studio/SKILL.md                     剪辑技能说明（三段式工作法/分镜模板/平台规格）
toolkit/cs.py                                    工具箱：18 个子命令
scripts/bootstrap.ps1 | .cmd                     运行时一键引导
scripts/install-into-dsh.ps1 | .cmd              技能安装进 DSH
scripts/run-toolkit.mjs                          跨平台启动器（node → venv python）
...
`selftest` **19/19 通过**（经 node 可移植启动器运行）：抠图（RGBA 输出、主体占比 79%、边缘过渡宽度量化）、**局部重绘改图**、中英文字与描边、白底 logo 自动抠、多图层合成、2000×2000 白底主图、接触表、主色提取；视频探针（横竖版各一）、抽帧、秒切/精确裁切、fade 转场拼接（10.45s，yuv420p）、贴字、中文字幕烧入、配乐淡入淡出、智能封面选帧、9:16 导出。

抠图模型实测（同一张商品图）：`fast`→u2net 0.15s / 边缘过渡 5.28px；`best`→birefnet 5.7s(CPU) / 2.76px。

## 已知边界（诚实声明）

- 生成式改图（换场景/换材质）**未实现**，属后续 T4 层；当前 `inpaint` 适合小面积缺陷修复。
- 任务级审美质量无法自动判定——本轮模型看不到图像时，验收靠导出帧/接触表交用户过目。
